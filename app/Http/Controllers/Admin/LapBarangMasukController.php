...
class LapBarangMasukController extends Controller
{
    ...
    public function excel(Request $request)
    {
        ...
        return response(view('Admin.Laporan.BarangMasuk.excel', $data))
            ->header('Content-Type', 'application/vnd.ms-excel')
            ->header('Content-Disposition', 'attachment; filename="Laporan_Barang_Masuk_'.($request->tglawal ? $request->tglawal.'-'.$request->tglakhir : 'semua').'.xls"');
    }
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            if ($request->tglawal == '') {
                $data = BarangmasukModel::select('tbl_barangmasuk.*', 'tbl_barang.barang_nama', 'tbl_customer.customer_nama')->leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangmasuk.barang_kode')->leftJoin('tbl_customer', 'tbl_customer.customer_id', '=', 'tbl_barangmasuk.customer_id')->orderBy('bm_id', 'DESC')->get();
            } else {
                $data = BarangmasukModel::select('tbl_barangmasuk.*', 'tbl_barang.barang_nama', 'tbl_customer.customer_nama')->leftJoin('tbl_barang', 'tbl_barang.barang_kode', '=', 'tbl_barangmasuk.barang_kode')->leftJoin('tbl_customer', 'tbl_customer.customer_id', '=', 'tbl_barangmasuk.customer_id')->whereBetween('bm_tanggal', [$request->tglawal, $request->tglakhir])->orderBy('bm_id', 'DESC')->get();
            }
            return DataTables::of($data)
                ...
                ->rawColumns(['tgl', 'customer', 'barang', 'timestamp'])->make(true);
        }
    }
}
